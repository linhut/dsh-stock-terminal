...
> 60s 缓存 + 在途去重；外汇无日K源，明确降级提示）。冒烟覆盖 A股/指数/港股/美股/加密/外汇/非法符号/405/跨域。
> 踩坑记录：① 腾讯 fqkline 的 param 需要 **三个** 逗号分隔空起止（`sym,day,,,N,qfq`），
> 少一个逗号上游返回 `param error`（HTTP 200，极易误判为网络问题）；② 腾讯 fqkline 对美股
> 只返回 2 根 K 线（端点限制），需回退 Yahoo chart；③ Binance klines 端点是把
> `/ticker/24hr` 替换为 `/klines`，不能直接拼接。
