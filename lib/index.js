...
async function fetchTencentKline(symbol, count) {
...
async function fetchBinanceKline(symbol, count) {
	for (const endpoint of BINANCE_ENDPOINTS) {
		// BINANCE_ENDPOINTS 指向 /ticker/24hr，K 线接口在同级 /klines
		const base = endpoint.replace(/\/ticker\/24hr$/, "");
		try {
			const response = await fetch(`${base}/klines?symbol=${encodeURIComponent(symbol)}&interval=1d&limit=${count}`, { signal: timeoutSignal(FETCH_TIMEOUT_MS) });
			if (!response.ok) continue;
			const rows = await response.json();
...
async function fetchYahooKline(symbol, count) {
	const range = count <= 23 ? "1mo" : count <= 66 ? "3mo" : count <= 140 ? "1y" : "2y";
	const response = await fetch(`${YAHOO_CHART_ENDPOINT}${encodeURIComponent(yahooSymbol(symbol))}?range=${range}&interval=1d`, {
 ...
 */
async function fetchKline(symbol, count) {
	const value = normalizeSymbol(symbol);
	const category = classifySymbol(value);
	if (category === "tencent") {
		let bars = [];
		try {
			bars = await fetchTencentKline(value, count);
		} catch {
			bars = [];
		}
		if (bars.length < KLINE_MIN_BARS && value.startsWith("us")) bars = await fetchYahooKline(value, count);
		if (bars.length === 0) throw new Error("kline-empty");
		return { symbol: value, bars };
	}
	if (category === "crypto") return { symbol: value, bars: await fetchBinanceKline(value, count) };
...
function klineRoute() {
	return {
		...
		handler: (req, res) => {
			...
			if (promise === undefined) {
				promise = fetchKline(symbol, count);
