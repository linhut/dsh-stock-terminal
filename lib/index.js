	...
	return {
		...
		handler: (req, res) => {
			...
			try {
				...
				symbols = query.split(",").map((s) => s.trim()).filter((s) => s.length > 0);
...
function suggestRoute() {
	return {
		...
		handler: (req, res) => {
			...
			fetch(`${SUGGEST_ENDPOINT}${encodeURIComponent(key)}`, { signal: timeoutSignal(FETCH_TIMEOUT_MS) })
				.then((response) => {
					if (!response.ok) throw new Error(`sina-${response.status}`);
					return response.arrayBuffer();
				})
				.then((buffer) => {
					const text = new TextDecoder("gbk").decode(new Uint8Array(buffer));
					const results = parseSuggestResponse(text);
					json(res, 200, { ok: true, results });
				})
				.catch((error) => json(res, 200, { ok: true, results: [], error: error.message }));
		}
	};
}
