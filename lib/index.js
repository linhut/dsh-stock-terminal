...
// 设置接入采用最新 DSH 官方 API：ctx.inject(["settings"]) 可选注入 +
// ctx.settings.installSection（dsh-settings ≥ 0.1.0-rc.x；旧版 installSettingsSection
// / settingsNamespace 已在 0.1.2-rc.1 移除，二者不再导出）。

import z from "@deepseek-ai/schemastery";

export const name = "dsh-stock-terminal";
export const inject = ["webServer"];

// 设置命名空间 schema，使本插件在系统设置 → 插件配置中可见；
// 同时作为插件 Config 导出，允许在 cordis.yml 里组合配置（schema 默认值在未配置时生效）。
const StockSettingsSchema = z.object({
  refreshMs: z.number().min(5000).max(120000).default(30000),
  showTape: z.boolean().default(true)
});

/** 组合配置（cordis.yml `config` 字段），与设置命名空间共享同一 schema。 */
export const Config = StockSettingsSchema;
const STOCK_NS = "dsh-stock";
...
// 分时是分钟级数据，TTL 收紧，兼顾去抖与时效
const MINUTE_CACHE_TTL_MS = 15000;
const KLINE_DEFAULT_COUNT = 120;
const KLINE_MIN_COUNT = 20;
const KLINE_MAX_COUNT = 320;
// 分时单日天然约 240 点；设上限防御上游异常超大响应（防御式边界）
const MAX_MINUTE_POINTS = 480;
const CACHE_MAX_ENTRIES = 128;
const quotesCache = new Map();
const suggestCache = new Map();
const klineCache = new Map();
const minuteCacheMap = new Map();
...
function minuteRoute() {
	return {
		kind: 'exact',
		path: '/plugins/dsh-stock/api/minute',
		handler: (req, res) => {
			if (!requireMethod(req, res, 'GET')) return;
			if (!requireSameOrigin(req, res)) return;
			const url = new URL(req.url ?? '/', 'http://x');
			const symbol = normalizeSymbol((url.searchParams.get('symbol') ?? '').trim());
			if (symbol === '' || classifySymbol(symbol) === null) {
				json(res, 400, { ok: false, error: `invalid-symbol: ${symbol}` });
				return;
			}
			// 短 TTL 缓存 + 在途去重，与 kline 路由同策略
			let promise = cacheGet(minuteCacheMap, symbol);
			if (promise === undefined) {
				promise = fetchTencentMinute(symbol);
				promise.catch(() => minuteCacheMap.delete(symbol));
				cacheSet(minuteCacheMap, symbol, promise, MINUTE_CACHE_TTL_MS);
			}
			promise.then(
				(result) => json(res, 200, { ok: true, ts: Date.now(), symbol: result.symbol, date: result.date, points: result.points.slice(0, MAX_MINUTE_POINTS), prevClose: result.prevClose }),
				// 失败也回 200 + ok:false，前端统一按“无数据”降级而不是抛网络错误
				(error) => json(res, 200, { ok: false, error: error instanceof Error ? error.message : String(error), symbol, points: [], prevClose: NaN })
			);
		}
	};
}
...
/** 腾讯分时接口：返回今日分钟级价格点 + 昨收价。 */
const TENCENT_MINUTE_ENDPOINT = 'https://web.ifzq.gtimg.cn/appstock/app/minute/query';
async function fetchTencentMinute(symbol) {
	const apiCode = normalizeSymbol(symbol);
	const url = TENCENT_MINUTE_ENDPOINT + '?code=' + encodeURIComponent(apiCode) + '&r=' + Math.random();
	const response = await fetch(url, { signal: timeoutSignal(FETCH_TIMEOUT_MS) });
	if (!response.ok) throw new Error('minute-http-' + response.status);
	const json = await response.json();
	if (!json || json.code !== 0) throw new Error('minute-api-error');
	const sd = json.data && json.data[apiCode];
	if (!sd || !sd.data || !Array.isArray(sd.data.data)) throw new Error('minute-no-data');
	const rawPoints = sd.data.data;
	const points = [];
	for (const line of rawPoints) {
		const parts = String(line).split(' ');
		if (parts.length < 2) continue;
		const hm = parts[0], p = toNumber(parts[1]), v = parts.length > 2 ? toNumber(parts[2]) : 0, a = parts.length > 3 ? toNumber(parts[3]) : 0;
		if (!/^\d{4}$/.test(hm) || !Number.isFinite(p)) continue;
		// 腾讯分时第 3 列为累计成交量、第 4 列为累计成交额，二者相除即官方均价
		points.push({ t: hm, p, v, a });
	}
	if (points.length === 0) throw new Error('minute-empty');
	// 解析昨收
	let prevClose = NaN;
	const qt = sd.qt && sd.qt[apiCode];
	if (Array.isArray(qt) && qt.length >= 35) {
		const price = toNumber(qt[3]);
		const chgPct = toNumber(qt[32]);
		if (Number.isFinite(price) && Number.isFinite(chgPct) && chgPct !== 0) {
			prevClose = price / (1 + chgPct / 100);
		} else if (Number.isFinite(price)) {
			prevClose = price;
		}
	}
	return { symbol, date: (sd.data.date || '') + '', points, prevClose };
}
...
/** 日志兜底：优先用 cordis logger，缺失时回退 console（测试/异常环境不崩）。 */
function logWarn(ctx, message, error) {
	const logger = ctx && typeof ctx.logger === "object" && ctx.logger !== null ? ctx.logger : null;
...
/**
 * 挂载路由与设置命名空间。失败只记日志、绝不让 GUI 启动失败（与 skin-center 同策略）。
 * @param ctx - cordis context。
 * @param config - 组合配置（cordis.yml config 字段，经 Config schema 校验）。
 */
export function apply(ctx, config = {}) {
	// 注册设置命名空间（系统设置 → 插件配置 可见）：使用最新官方 API —
	// ctx.inject(["settings"]) 可选注入，设置服务存在时才注册；失败只记日志，绝不影响启动。
	try {
		ctx.inject(["settings"], (settingsCtx) => {
			try {
				settingsCtx.settings.installSection(ctx, STOCK_NS, StockSettingsSchema, config, {
					setSource: () => {},
					onChange: () => {}
				});
			} catch (error) {
				logWarn(ctx, "[ui-skin-stock] settings registration failed", error);
			}
		});
	} catch (error) {
		logWarn(ctx, "[ui-skin-stock] settings registration failed", error);
	...
	try {
		ctx.effect(() => {
			const disposers = [];
			try {
				for (const route of [quotesRoute(), suggestRoute(), klineRoute(), minuteRoute()]) disposers.push(ctx.webServer.register(route));
			} catch (error) {
		...
		logWarn(ctx, "[ui-skin-stock] route registration failed", error);
	}
}