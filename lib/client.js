...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			`.dsh-stk-panel{z-index:1000002;position:fixed;right:12px;bottom:38px;width:min(600px,calc(100vw - 24px));height:75vh;max-height:calc(100vh - 150px);min-width:400px;min-height:260px;resize:both;overflow:hidden;background:var(--stk-panel);border:1px solid var(--stk-border);box-shadow:0 8px 30px #17202d33;color:var(--stk-text);font:400 13px/1.5 var(--dsw-font-family);transform:scaleX(-1) scaleY(-1);transform-origin:top left}`,
			`.dsh-stk-panel[hidden]{display:none}`,
			`.dsh-stk-panelContent{display:flex;flex-direction:column;width:100%;height:100%;transform:scaleX(-1) scaleY(-1)}`,
	...
	function apply(ctx) {
		...
		const panelContent = document.createElement("div"); panelContent.className = "dsh-stk-panelContent";
		...
		panelContent.append(panelHead, panelBody);
