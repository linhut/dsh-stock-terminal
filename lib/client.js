...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function StockSettingsCard(props) {
			...
			const hRefresh = (e) => { const v = Number(e.target.value); setRefreshMs(v); state.settings.refreshMs = v; persistSettings(); if (state.settingsCtrl) state.settingsCtrl.set("refreshMs", v); };
			const hTape = (e) => { const v = e.target.checked; setShowTape(v); state.settings.showTape = v; persistSettings(); if (state.settingsCtrl) state.settingsCtrl.set("showTape", v); };
			const hWlInput = (e) => {
			...
			const addWl = (sym) => {
				if (!sym) return;
				if (!state.watchlist.includes(sym)) { state.watchlist.push(sym); persistWatchlist(); setWlItems([...state.watchlist]); }
			...
			const addPos = () => {
				const sym = posSym.trim(), qty = Number(posQty), cost = Number(posCost);
		...
		function apply(ctx) {
			...
			wlInput.addEventListener("input", () => {
				...
				clearTimeout(window._suggestTimer);
				window._suggestTimer = window.setTimeout(async () => {
					const apiResults = await fetchSuggestions(q);
					if (document.activeElement !== wlInput) return; // 用户已离开
