...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			`@keyframes dsh-stk-tapeMove{0%{transform:translate(0)}to{transform:translate(-50%)}}`,
			// ---- statusbar ----
		...
		// stock popup (K线弹窗)
			// 盯盘提醒（alert）
			`.dsh-stk-statusbarAlert{align-items:center;gap:4px;display:inline-flex;color:var(--stk-dim);cursor:default}.dsh-stk-statusbarAlert[data-active=1]{color:var(--stk-brand);font-weight:600}`,
			`.dsh-stk-alertCell{font-size:12px;white-space:nowrap;text-align:center!important}.dsh-stk-alertCell:first-child{text-align:left}`,
			`.dsh-stk-alertBadge{display:inline-flex;align-items:center;gap:4px;cursor:pointer;font:500 11px/1 var(--ds-font-family-code);font-variant-numeric:tabular-nums;padding:3px 6px;border-radius:3px;border:1px solid var(--stk-border);color:var(--stk-dim);background:var(--stk-hover);white-space:nowrap}.dsh-stk-alertBadge:hover{border-color:var(--stk-brand);color:var(--stk-text)}`,
			`.dsh-stk-alertBadge[data-state=armed]{color:var(--stk-brand);border-color:var(--stk-brand)}`,
			`.dsh-stk-alertBadge[data-state=fired]{color:var(--stk-warn);border-color:var(--stk-warn)}`,
			`.dsh-stk-alertModal{z-index:1000006}`,
			`.dsh-stk-alertCard{width:min(400px,calc(100vw - 48px));max-width:400px;height:auto;display:flex;flex-direction:column;background:var(--stk-panel);border:1px solid var(--stk-border);box-shadow:0 12px 40px #17202d55;color:var(--stk-text);font:400 13px/1.5 var(--dsw-font-family)}`,
			`.dsh-stk-alertBody{flex:1;min-height:0;padding:14px 12px;display:flex;flex-direction:column;gap:8px;overflow-y:auto}`,
			`.dsh-stk-alertBody .dsh-stk-formrow label{width:58px;flex:none}`,
			`.dsh-stk-alertHint{color:var(--stk-dim);font-size:11px;line-height:1.6;margin-top:2px}`,
			`.dsh-stk-alertFoot{flex:none;display:flex;gap:8px;padding:10px 12px;border-top:1px solid var(--stk-border);justify-content:flex-end}`,
		...
		`.dsh-stk-modalCard{width:min(820px,calc(100vw - 48px));height:min(520px,calc(100vh - 96px));display:flex;flex-direction:column;background:var(--stk-panel);border:1px solid var(--stk-border);box-shadow:0 12px 40px #17202d55;color:var(--stk-text);font:400 13px/1.5 var(--dsw-font-family);position:relative}.dsh-stk-modalCard[data-size]{width:auto;height:auto;min-width:360px;min-height:240px}`,
		...
		`.dsh-stk-modalBody{flex:1;min-height:0;position:relative;display:flex;flex-direction:column}`,
		`.dsh-stk-modalBody canvas{display:block;width:100%;height:100%;flex:1;min-height:160px}`,
		...
		`.dsh-stk-modalFoot{flex:none;display:flex;gap:12px;padding:6px 12px;border-top:1px solid var(--stk-border);font:500 11px/1 var(--ds-font-family-code);font-variant-numeric:tabular-nums;align-items:center;flex-wrap:wrap}.dsh-stk-modalResize{position:absolute;right:0;bottom:0;width:16px;height:16px;cursor:nwse-resize;background:linear-gradient(135deg,transparent 50%,var(--stk-dim) 50%,var(--stk-dim) 55%,transparent 55%,transparent 70%,var(--stk-dim) 70%,var(--stk-dim) 75%,transparent 75%);user-select:none;z-index:1}.dsh-stk-modalResize:hover{opacity:.8}`,
		// toast
		...
		const tagId = "dsh-stock-terminal/stock.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-stock-terminal";
		...
		function parseTencentRow(raw) {
			...
			return { name: f[1] !== void 0 && f[1] !== "" ? f[1] : f[2] ?? "", price, prevClose: toNumber(f[4]), change: toNumber(f[31]), changePct: toNumber(f[32]), high: toNumber(f[33]), low: toNumber(f[34]), open: toNumber(f[5]), volume: toNumber(f[6]) };
		}
			...
			for (const endpoint of BINANCE_ENDPOINTS) {
				try {
					...
					for (const row of rows) {
						...
						out.set(symbol, { symbol, name: CRYPTO_NAMES[symbol] ?? symbol, price, changeAbs: toNumber(row.priceChange), changePct: toNumber(row.priceChangePercent), high: toNumber(row.highPrice), low: toNumber(row.lowPrice), open: toNumber(row.openPrice), volume: toNumber(row.volume), source: "binance" });
					}
	...
	/** 分时画布几何（与绘制共用，供命中测试/悬浮定位）。 */
	function minuteLayout(canvas) {
		const cssW = canvas.clientWidth || 640, cssH = canvas.clientHeight || 320;
		return { padL: 8, padR: 58, padT: 10, padB: 18, cssW, cssH };
	}

	/** Canvas 绘制分时图：价格线 + 昨收基准 + 均价线 + 分时成交量 + 十字光标悬浮。 */
	function drawMinuteChart(canvas, data, hover) {
		var points = data.points, prevClose = data.prevClose;
		var n = points.length;
		if (n < 2) return;
		var L = minuteLayout(canvas);
		var cssW = L.cssW, cssH = L.cssH;
		if (cssW <= 0 || cssH <= 0) return;
		var dpr = window.devicePixelRatio || 1;
		canvas.width = Math.round(cssW * dpr); canvas.height = Math.round(cssH * dpr);
		var g = canvas.getContext("2d"); if (g === null) return;
		g.setTransform(dpr, 0, 0, dpr, 0, 0); g.clearRect(0, 0, cssW, cssH);
		var style = getComputedStyle(document.body);
		var cUp = style.getPropertyValue("--stk-up").trim() || "#e02e3d";
		var cDown = style.getPropertyValue("--stk-down").trim() || "#089981";
		var cDim = style.getPropertyValue("--stk-dim").trim() || "#6b7788";
		var cBorder = style.getPropertyValue("--stk-border").trim() || "#d4dce5";
		var cPanel = style.getPropertyValue("--stk-panel").trim() || "#fff";
		var cAvg = style.getPropertyValue("--stk-avg").trim() || "#d9a13b";
		var codeFont = style.getPropertyValue("--ds-font-family-code").trim() || "monospace";
		var padL = L.padL, padR = L.padR, padT = L.padT, padB = L.padB;
		var volH = Math.min(48, Math.max(24, cssH * 0.14)), volGap = 12;
		var priceH = cssH - padT - padB - volH - volGap;
		var plotW = cssW - padL - padR;
		if (priceH <= 10 || plotW <= 10) return;
		// 腾讯分时第 3/4 列为累计值，单分钟成交量 = 累计量差分
		var vols = new Array(n);
		var avgOk = true;
		for (var i = 0; i < n; i++) { var v0 = points[i].v; vols[i] = i > 0 ? Math.max(0, v0 - points[i - 1].v) : Math.max(0, v0); if (!(points[i].v > 0) || !Number.isFinite(points[i].a)) avgOk = false; }
		var hi = -Infinity, lo = Infinity, maxVol = 0;
		for (var i = 0; i < n; i++) { var p = points[i].p; if (p > hi) hi = p; if (p < lo) lo = p; var dv = vols[i]; if (dv > maxVol) maxVol = dv; }
		if (!Number.isFinite(hi) || !Number.isFinite(lo) || hi === lo) { hi = hi + 1; lo = lo - 1; }
		if (Number.isFinite(prevClose) && prevClose > 0) { hi = Math.max(hi, prevClose); lo = Math.min(lo, prevClose); }
		var span = Math.max(hi - lo, 0.01);
		hi += span * 0.05; lo -= span * 0.05;
		var yPrice = function(p) { return padT + priceH * (1 - (p - lo) / (hi - lo)); };
		var step = plotW / (n - 1);
		var xPos = function(i) { return padL + step * i; };
		var volTop = padT + priceH + volGap, volBase = volTop + volH;
		g.font = "10px " + codeFont; g.textBaseline = "middle";
		for (var k = 0; k <= 4; k++) {
			var p = lo + (hi - lo) * k / 4, y = yPrice(p);
			g.strokeStyle = cBorder; g.globalAlpha = 0.45;
			g.beginPath(); g.moveTo(padL, y); g.lineTo(padL + plotW, y); g.stroke();
			g.globalAlpha = 1; g.fillStyle = cDim; g.textAlign = "left";
			g.fillText(klinePriceText(p), padL + plotW + 6, y);
		}
		if (Number.isFinite(prevClose) && prevClose > 0) {
			var y0 = yPrice(prevClose);
			g.strokeStyle = "#8b96a5"; g.globalAlpha = 0.7; g.lineWidth = 1; g.setLineDash([3, 3]);
			g.beginPath(); g.moveTo(padL, y0); g.lineTo(padL + plotW, y0); g.stroke();
			g.setLineDash([]); g.globalAlpha = 1;
			g.fillStyle = cDim; g.textAlign = "left"; g.fillText(klinePriceText(prevClose), padL + plotW + 6, y0 + 12);
		}
		// 时间轴：按数据实际点位取四等分标签（兼容 A股午休 / 港股 / 美股时段差异）
		g.textAlign = "center"; g.fillStyle = cDim; g.font = "10px " + codeFont;
		for (var t = 0; t <= 4; t++) {
			var idx = Math.round((n - 1) * t / 4);
			var tl = points[idx].t;
			g.fillText(tl.slice(0, 2) + ":" + tl.slice(2), xPos(idx), cssH - padB / 2);
		}
		// 成交量（单分钟增量）
		for (var i3 = 0; i3 < n; i3++) {
			var p2 = points[i3], up = p2.p >= prevClose;
			var h = maxVol > 0 ? vols[i3] / maxVol * volH : 0;
			g.fillStyle = up ? cUp : cDown; g.globalAlpha = 0.4;
			g.fillRect(xPos(i3) - 1, volBase - Math.max(h, 0.5), Math.max(2, step - 1), Math.max(h, 0.5));
		}
		g.globalAlpha = 1;
		...
		g.globalAlpha = 1;
		var last = points[n - 1];
		var trendColor = last.p >= prevClose ? cUp : cDown;
		// 均价线（累计成交额/累计成交量，官方口径；与价格线同图对比强弱）
		if (avgOk) {
			g.strokeStyle = cAvg; g.globalAlpha = 0.9; g.lineWidth = 1;
			g.beginPath();
			var started = false;
			for (var ia = 0; ia < n; ia++) {
				var avg = points[ia].v > 0 && Number.isFinite(points[ia].a) ? points[ia].a / points[ia].v : NaN;
				if (!Number.isFinite(avg)) continue;
				var ya = yPrice(avg);
				if (!started) { g.moveTo(xPos(ia), ya); started = true; } else g.lineTo(xPos(ia), ya);
			}
			if (started) g.stroke();
		}
		g.beginPath(); g.moveTo(xPos(0), yPrice(points[0].p));
		for (var i4 = 1; i4 < n; i4++) g.lineTo(xPos(i4), yPrice(points[i4].p));
		g.lineTo(xPos(n - 1), volTop); g.lineTo(xPos(0), volTop); g.closePath();
		g.fillStyle = trendColor; g.globalAlpha = 0.08; g.fill();
		g.globalAlpha = 1; g.strokeStyle = trendColor; g.lineWidth = 1.5;
		g.beginPath(); g.moveTo(xPos(0), yPrice(points[0].p));
		for (var i5 = 1; i5 < n; i5++) g.lineTo(xPos(i5), yPrice(points[i5].p));
		g.stroke();
		// 悬浮十字光标 + 信息框（参考 lightweight-charts crosshair 交互）
		if (hover !== void 0 && hover >= 0 && hover < n) {
			var hp = points[hover];
			var hx = xPos(hover), hy = yPrice(hp.p);
			var hUp = hp.p >= prevClose;
			var hColor = hUp ? cUp : cDown;
			g.strokeStyle = "#8b96a5"; g.globalAlpha = 0.55; g.lineWidth = 1; g.setLineDash([4, 3]);
			g.beginPath(); g.moveTo(hx, padT); g.lineTo(hx, volBase); g.stroke();
			g.beginPath(); g.moveTo(padL, hy); g.lineTo(padL + plotW, hy); g.stroke();
			g.setLineDash([]); g.globalAlpha = 1;
			g.fillStyle = hColor; g.beginPath(); g.arc(hx, hy, 3, 0, Math.PI * 2); g.fill();
			g.fillStyle = hColor; g.globalAlpha = 0.55;
			var hv = maxVol > 0 ? vols[hover] / maxVol * volH : 0;
			g.fillRect(xPos(hover) - 1, volBase - Math.max(hv, 0.5), Math.max(2, step - 1), Math.max(hv, 0.5));
			g.globalAlpha = 1;
			var hChg = Number.isFinite(prevClose) && prevClose > 0 ? (hp.p / prevClose - 1) * 100 : NaN;
			var hAvg = hp.v > 0 && Number.isFinite(hp.a) ? hp.a / hp.v : NaN;
			var lines = [
				"时间  " + hp.t.slice(0, 2) + ":" + hp.t.slice(2),
				"价格  " + klinePriceText(hp.p) + (Number.isFinite(hChg) ? (hChg > 0 ? "  +" : "  ") + hChg.toFixed(2) + "%" : ""),
				"成交量  " + Math.round(vols[hover]),
				"均价  " + (Number.isFinite(hAvg) ? klinePriceText(hAvg) : "--")
			];
			var bw = 132, bh = 15 * lines.length + 10;
			var bx = hx + 14 + bw > padL + plotW ? hx - bw - 14 : hx + 14;
			var by = Math.max(padT, Math.min(volBase - bh, hy - bh / 2));
			g.fillStyle = cPanel; g.globalAlpha = 0.96;
			g.fillRect(bx, by, bw, bh);
			g.strokeStyle = cBorder; g.globalAlpha = 0.7; g.strokeRect(bx, by, bw, bh);
			g.globalAlpha = 1; g.fillStyle = cDim; g.textAlign = "left"; g.textBaseline = "middle"; g.font = "10px " + codeFont;
			for (var li = 0; li < lines.length; li++) g.fillText(lines[li], bx + 6, by + 7.5 + li * 15);
		}
		var lastY = yPrice(last.p);
		g.fillStyle = cPanel; g.fillRect(padL + plotW, lastY - 7, padR, 14);
		g.fillStyle = trendColor; g.textAlign = "left"; g.textBaseline = "middle";
		g.fillText(klinePriceText(last.p), padL + plotW + 6, lastY);
		var chgPct = Number.isFinite(prevClose) && prevClose > 0 ? ((last.p / prevClose - 1) * 100) : NaN;
		g.fillStyle = cPanel; g.globalAlpha = 0.92;
		g.fillRect(padL + 2, padT - 4, 150, 20);
		g.globalAlpha = 1;
		g.fillStyle = trendColor; g.textAlign = "left"; g.textBaseline = "middle";
		g.fillText(klinePriceText(last.p) + "  " + (Number.isFinite(chgPct) ? (chgPct > 0 ? "+" : "") + chgPct.toFixed(2) + "%" : "--"), padL + 8, padT + 6);
	}
function klinePriceText(p) { return p >= 1000 ? p.toFixed(0) : p.toFixed(2); }
		/** K线画布几何布局（每帧/每次交互重算，供绘制与命中测试共用）。 */
	function klineLayout(canvas, bars, view) {
		const cssW = canvas.clientWidth || 640, cssH = canvas.clientHeight || 320;
		const padL = 8, padR = 58, padT = 10, padB = 18;
		const volH = Math.min(64, Math.max(34, cssH * 0.18)), volGap = 16;
		const priceH = cssH - padT - padB - volH - volGap;
		const plotW = cssW - padL - padR;
		const count = Math.max(2, Math.min(view.count, bars.length));
		const start = Math.max(0, Math.min(bars.length - count, view.start));
		const slice = bars.slice(start, start + count);
		let hi = -Infinity, lo = Infinity, maxVol = 0;
		for (const b of slice) { if (b.h > hi) hi = b.h; if (b.l < lo) lo = b.l; if (b.v > maxVol) maxVol = b.v; }
		if (!Number.isFinite(hi) || !Number.isFinite(lo) || hi === lo) { hi = hi + 1; lo = lo - 1; }
		const rawSpan = hi - lo;
		if (!Number.isFinite(rawSpan) || rawSpan <= 0) { hi += 1; lo -= 1; } else { hi += rawSpan * 0.05; lo -= rawSpan * 0.05; }
		const step = plotW / count;
		const bodyW = Math.max(1, Math.min(12, step * 0.62));
		return {
			cssW, cssH, padL, padR, padT, padB, volH, volGap, priceH, plotW,
			start, count, slice, hi, lo, maxVol, step, bodyW,
			yPrice: (p) => padT + priceH * (1 - (p - lo) / (hi - lo)),
			xCenter: (i) => padL + step * (i + 0.5),
			volTop: padT + priceH + volGap,
			volBase: padT + priceH + volGap + volH
		};
	}
	function klineVolText(v) { if (!Number.isFinite(v)) return "--"; if (v >= 1e8) return (v / 1e8).toFixed(2) + "亿"; if (v >= 1e4) return (v / 1e4).toFixed(2) + "万"; return String(Math.round(v)); }
	/**
	 * Canvas 绘制K线：蜡烛 + MA5/MA10 + 成交量副图 + 网格/轴/十字光标。
	 * view: { start, count } 可视窗口；hover: 悬浮 bar 绝对索引（-1 不显示）。
	 * 交互范式参考 tradingview/lightweight-charts 与 klinecharts（不引入第三方依赖，原生 canvas 实现）。
	 */

	/** 获取今日分时数据（腾讯分钟接口）。 */
	async function fetchMinute(symbol, timeoutMs) {
		if (timeoutMs === void 0) timeoutMs = 1e4;
		const controller = new AbortController(); const timer = setTimeout(function() { controller.abort(); }, timeoutMs);
		try {
			const response = await fetch("/plugins/dsh-stock/api/minute?symbol=" + encodeURIComponent(symbol), { signal: controller.signal });
			if (!response.ok) throw new Error("http-" + response.status);
			const json = await response.json();
			if (!json.ok) throw new Error(json.error || "minute-fail");
			return { points: json.points, prevClose: json.prevClose, date: json.date };
		} finally { clearTimeout(timer); }
	}
	function drawKlineChart(canvas, bars, view, hover) {
		const L = klineLayout(canvas, bars, view || { start: 0, count: bars.length });
		if (L.count < 2 || L.priceH <= 10 || L.plotW <= 10) return;
		const dpr = window.devicePixelRatio || 1;
		canvas.width = Math.round(L.cssW * dpr); canvas.height = Math.round(L.cssH * dpr);
		const g = canvas.getContext("2d"); if (g === null) return;
		g.setTransform(dpr, 0, 0, dpr, 0, 0); g.clearRect(0, 0, L.cssW, L.cssH);
		...
		const cPanel = style.getPropertyValue("--stk-panel").trim() || "#fff";
		const codeFont = style.getPropertyValue("--ds-font-family-code").trim() || "monospace";
		// 网格 + 价格轴
		g.font = `10px ${codeFont}`; g.textBaseline = "middle";
		for (let k = 0; k <= 4; k += 1) {
			const p = L.lo + (L.hi - L.lo) * k / 4, y = L.yPrice(p);
			g.strokeStyle = cBorder; g.globalAlpha = 0.45;
			g.beginPath(); g.moveTo(L.padL, y); g.lineTo(L.padL + L.plotW, y); g.stroke();
			g.globalAlpha = 1; g.fillStyle = cDim; g.textAlign = "left";
			g.fillText(klinePriceText(p), L.padL + L.plotW + 6, y);
		}
		// 日期轴（可视窗口首/中/尾）
		g.textAlign = "center"; g.fillStyle = cDim;
		const mid = Math.floor(L.count / 2);
		for (const i of [0, mid, L.count - 1]) { const b = L.slice[i]; if (b) g.fillText(String(b.t).slice(0, 10), L.xCenter(i), L.cssH - L.padB / 2); }
		// 成交量副图
		for (let i = 0; i < L.count; i += 1) {
			const b = L.slice[i], up = b.c >= b.o;
			const h = L.maxVol > 0 ? b.v / L.maxVol * L.volH : 0;
			g.fillStyle = up ? cUp : cDown; g.globalAlpha = 0.5;
			g.fillRect(L.xCenter(i) - L.bodyW / 2, L.volBase - Math.max(h, 0.5), L.bodyW, Math.max(h, 0.5));
		}
		g.globalAlpha = 1;
		// 价格/成交量分隔线
		g.strokeStyle = cBorder; g.globalAlpha = 0.6;
		g.beginPath(); g.moveTo(L.padL, L.volTop - L.volGap / 2); g.lineTo(L.padL + L.plotW, L.volTop - L.volGap / 2); g.stroke();
		g.globalAlpha = 1;
		// 蜡烛（先影线后实体）
		for (let i = 0; i < L.count; i += 1) {
			const b = L.slice[i], up = b.c >= b.o, x = L.xCenter(i);
			g.strokeStyle = up ? cUp : cDown; g.lineWidth = 1;
			g.beginPath(); g.moveTo(x, L.yPrice(b.h)); g.lineTo(x, L.yPrice(b.l)); g.stroke();
			g.fillStyle = up ? cUp : cDown;
			const top = Math.min(L.yPrice(b.o), L.yPrice(b.c));
			g.fillRect(x - L.bodyW / 2, top, L.bodyW, Math.max(Math.abs(L.yPrice(b.c) - L.yPrice(b.o)), 1));
		}
		// MA5 / MA10（跨窗口边界取未显示历史计算，保证边缘均线连续）
		const maSeries = (period) => {
			const out = new Array(L.count).fill(NaN);
			for (let i = 0; i < L.count; i += 1) {
				const endIdx = L.start + i;
				if (endIdx < period - 1) continue;
				let s = 0; let valid = true;
				for (let j = endIdx - period + 1; j <= endIdx; j += 1) { const b = bars[j]; if (!b) { valid = false; break; } s += b.c; }
				if (valid) out[i] = s / period;
			}
			return out;
		};
		const drawMA = (series, color) => {
			...
			for (let i = 0; i < L.count; i += 1) {
				...
				const x = L.xCenter(i), y = L.yPrice(v);
				if (!started) { g.moveTo(x, y); started = true; } else g.lineTo(x, y);
			}
			g.stroke();
		};
		drawMA(maSeries(5), "#c08a35"); drawMA(maSeries(10), "#4a7ede");
		// 十字光标 + OHLC 浮层
		if (hover !== void 0 && hover >= 0 && hover >= L.start && hover < L.start + L.count) {
			const i = hover - L.start, b = bars[hover], x = L.xCenter(i), y = L.yPrice(b.c);
			const dot = [3, 3];
			g.strokeStyle = cDim; g.globalAlpha = 0.75; g.lineWidth = 1; g.setLineDash(dot);
			g.beginPath(); g.moveTo(x, L.padT); g.lineTo(x, L.volBase); g.moveTo(L.padL, y); g.lineTo(L.padL + L.plotW, y); g.stroke();
			g.setLineDash([]); g.globalAlpha = 1;
			// 右侧价格标签块
			g.fillStyle = cPanel; g.fillRect(L.padL + L.plotW, y - 7, L.padR, 14);
			g.fillStyle = cDim; g.textAlign = "left"; g.textBaseline = "middle";
			g.fillText(klinePriceText(b.c), L.padL + L.plotW + 6, y);
			// 顶部 OHLC 浮层
			const up = b.c >= b.o;
			g.fillStyle = cPanel; g.globalAlpha = 0.92;
			g.fillRect(L.padL + 2, L.padT - 4, 200, 34);
			g.globalAlpha = 1;
			g.fillStyle = up ? cUp : cDown; g.textAlign = "left"; g.textBaseline = "middle";
			g.fillText(`${String(b.t).slice(0, 10)}  开${klinePriceText(b.o)} 高${klinePriceText(b.h)} 低${klinePriceText(b.l)} 收${klinePriceText(b.c)}`, L.padL + 8, L.padT + 6);
			g.fillStyle = cDim;
			g.fillText(`量${klineVolText(b.v)}   涨跌 ${b.o > 0 ? ((b.c / b.o - 1) * 100).toFixed(2) : "--"}%`, L.padL + 8, L.padT + 20);
		}
	}	//#endregion
	...
	// dsh-manager 品牌 logo（透明鲸鱼）：直接引用官方站点资源（https://dsh.linhut.cn/）。
	// 皮肤 bundle 是单文件注入（无静态资源路由），改用官方 CDN 直链避免内嵌体积与维护成本。
	const LOGO_URL = "https://dsh.linhut.cn/assets/images/logo.png";
	const FAVICON_URL = "https://dsh.linhut.cn/assets/images/favicon.png";
	function placeholderQuote(symbol) { return { symbol, name: symbol, price: NaN, changePct: NaN, changeAbs: NaN, high: NaN, low: NaN, open: NaN, volume: NaN, source: "tencent" }; }
	...
	function toast(message) {
		...
		// 盯盘通知：Toast 始终提示 + 浏览器桌面通知（Notification API）
	let notifPermission = typeof Notification !== "undefined" ? Notification.permission : "denied";
	function ensureNotifPermission() {
		if (typeof Notification === "undefined" || notifPermission !== "default") return;
		try {
			const r = Notification.requestPermission();
			if (r && typeof r.then === "function") r.then((p) => { notifPermission = p; });
		} catch {}
	}
	function notifyAlert(name, symbol, dir, level, price) {
		const dirText = dir === "high" ? "到达盯盘高价" : "跌破盯盘低价";
		const msg = `\uD83D\uDD14 ${name}（${symbol}）${dirText} ${klinePriceText(level)}，现价 ${klinePriceText(price)}`;
		toast(msg);
		if (typeof Notification !== "undefined" && notifPermission === "granted") {
			try {
				const n = new Notification("股市行情 · 盯盘提醒", { body: `${name}（${symbol}）${dirText} ${klinePriceText(level)}，现价 ${klinePriceText(price)}`, tag: "dsh-stock-alert" });
	...
	function updateQuoteLine(container, quote) {
		...
		const chgAmt = Number.isFinite(quote.changeAbs) ? `${quote.changeAbs > 0 ? "+" : ""}${quote.changeAbs.toFixed(2)}` : "";
		chg.textContent = `${trend === "up" ? "▲" : trend === "down" ? "▼" : ""}${chgAmt !== "" ? chgAmt + " " : ""}${pctText(trend, quote.changePct)}`;
	...
	function apply(ctx) {
		...
		let disposed = false;
		// ---- 初始化 settingsScope（DSH settings 服务 = 权威持久化源，settings.yaml 落盘）----
		try {
			const scope = ctx.get("settingsScope");
			if (scope) state.settingsCtrl = scope.bind({ namespace: "dsh-stock" });
		} catch {}
		// ---- 启动时从 settings 服务回读持久化配置并订阅变化 ----
		// 之前配置只存浏览器 localStorage（per-origin，更新插件/换浏览器即丢）；现在以
		// DSH settings 服务（dsh-settings-file → $DSH_HOME/settings.yaml）为权威源，
		// localStorage 仅作启动缓存。这样更新插件、清理浏览器数据后配置仍能恢复。
		const applyStoredSettings = () => {
			try {
				const snap = state.settingsCtrl && state.settingsCtrl.getSnapshot();
				if (snap && snap.status === "ready" && snap.value && typeof snap.value === "object") {
					const v = snap.value;
					if (typeof v.refreshMs === "number") state.settings.refreshMs = v.refreshMs;
					if (typeof v.showTape === "boolean") state.settings.showTape = v.showTape;
					persistSettings(); // localStorage 仅作缓存，权威值在 settings 服务
				}
			} catch {}
		};
		applyStoredSettings();
		try {
			if (state.settingsCtrl && typeof state.settingsCtrl.subscribe === "function") {
				state.settingsCtrl.subscribe(() => {
					if (disposed) return;
					applyStoredSettings();
					startTimers(); renderTape(); renderPosChip();
				});
			}
		} catch {}
		...
		const brand = document.createElement("span"); brand.className = "dsh-stk-titlebarIcon";
		const brandImg = document.createElement("img"); brandImg.src = LOGO_URL; brandImg.alt = ""; brandImg.style.cssText = "width:14px;height:14px;display:block;object-fit:contain"; brand.append(brandImg);
		...
		const alertCell = document.createElement("span"); alertCell.className = "dsh-stk-statusbarAlert"; alertCell.textContent = "\uD83D\uDD14 0"; alertCell.title = "激活盯盘提醒数";
		...
		statusbar.append(leftGroup, idxGroup, alertCell, pnlCell, toggleBtn, setBtn, rightGroup);
		// ---- panel ----
		...
		const cancelPosBtn = document.createElement("button"); cancelPosBtn.type = "button"; cancelPosBtn.className = "dsh-stk-btn"; cancelPosBtn.textContent = "取消";
				const fAlertH = document.createElement("div"); fAlertH.className = "dsh-stk-formrow";
		const lAlertH = document.createElement("label"); lAlertH.textContent = "盯盘高价"; lAlertH.style.width = "62px"; const iAlertH = document.createElement("input"); iAlertH.type = "number"; iAlertH.min = "0"; iAlertH.step = "any"; iAlertH.placeholder = "涨到此价提醒（可留空）"; fAlertH.append(lAlertH, iAlertH);
		const fAlertL = document.createElement("div"); fAlertL.className = "dsh-stk-formrow";
		const lAlertL = document.createElement("label"); lAlertL.textContent = "盯盘低价"; lAlertL.style.width = "62px"; const iAlertL = document.createElement("input"); iAlertL.type = "number"; iAlertL.min = "0"; iAlertL.step = "any"; iAlertL.placeholder = "跌到此价提醒（可留空）"; fAlertL.append(lAlertL, iAlertL);
		fBtns.append(savePosBtn, cancelPosBtn); posForm.append(fSym, fQty, fCost, fAlertH, fAlertL, fBtns);
		...
		for (const th of ["代码/名称", "数量", "成本", "现价", "市值", "盈亏", "盈亏%", "提醒", "操作"]) { const cell = document.createElement("th"); cell.textContent = th; headTr.append(cell); }
		...
		// ---- favicon / title ----
		const favicon = document.createElement("link"); favicon.rel = "icon"; favicon.href = FAVICON_URL;
	...
	function renderTape() {
		...
		track.style.animationPlayState = state.settings.showTape ? "" : "paused";
	}
		...
		function renderAlertStatus() {
			...
			for (const p of state.positions) {
				const hi = Number.isFinite(p.alertHigh) && p.alertHigh > 0 && p.fired !== "high";
				const lo = Number.isFinite(p.alertLow) && p.alertLow > 0 && p.fired !== "low";
				if (hi || lo) armed += 1;
			}
			alertCell.textContent = `\uD83D\uDD14 ${armed}`;
			alertCell.title = armed > 0 ? `${armed} 个持仓盯盘中，点击持仓表格\uD83D\uDD14可设置` : "无激活盯盘，点击持仓行\uD83D\uDD14设置";
			if (armed > 0) alertCell.dataset.active = "1"; else delete alertCell.dataset.active;
		}
		...
		function renderWatchlist() {
			...
			state.watchlist.forEach((symbol, index) => {
				...
				const chgAmt = Number.isFinite(quote.changeAbs) ? `${quote.changeAbs > 0 ? "+" : ""}${quote.changeAbs.toFixed(2)}` : "";
		chg.textContent = `${trend === "up" ? "▲" : trend === "down" ? "▼" : ""}${chgAmt !== "" ? chgAmt + " " : ""}${pctText(trend, quote.changePct)}`; applyTrend(chg, trend);
		...
		function renderPositions() {
			...
			if (state.positions.length === 0) {
				const tr = document.createElement("tr"); const td = document.createElement("td"); td.colSpan = 9; td.className = "dsh-stk-empty"; td.textContent = "暂无持仓。点击「添加持仓」记录。"; tr.append(td); posTbody.append(tr); posSummary.textContent = ""; return;
			}
			for (const pos of state.positions) {
				...
				const tdAlert = document.createElement("td"); tdAlert.className = "dsh-stk-alertCell";
				const hasHigh = Number.isFinite(pos.alertHigh) && pos.alertHigh > 0;
				const hasLow = Number.isFinite(pos.alertLow) && pos.alertLow > 0;
				if (hasHigh || hasLow) {
					const badge = document.createElement("span"); badge.className = "dsh-stk-alertBadge"; badge.dataset.act = "alert"; badge.title = "点击修改/重置盯盘价";
					badge.dataset.state = pos.fired ? "fired" : "armed";
					const parts = [];
					if (hasHigh) parts.push("▲" + Number(pos.alertHigh).toFixed(2));
					if (hasLow) parts.push("▼" + Number(pos.alertLow).toFixed(2));
					badge.textContent = parts.join(" ");
					if (pos.fired) badge.title = "已触发" + (pos.fired === "high" ? "高价" : "低价") + "提醒，点击重置";
					tdAlert.append(badge);
				} else {
					const setBtn = document.createElement("button"); setBtn.type = "button"; setBtn.className = "dsh-stk-btnDanger"; setBtn.dataset.act = "alert"; setBtn.textContent = "\uD83D\uDD14";
					setBtn.title = "设置盯盘价"; tdAlert.append(setBtn);
				}
				...
				tdOps.append(editBtn, delBtn); tr.append(tdName, tdQty, tdCost, tdPrice, tdMv, tdPnl, tdPnlPct, tdAlert, tdOps); tr.dataset.symbol = pos.symbol; posTbody.append(tr);
			}
		...
		function renderAll() { renderTape(); renderChips(); renderPosChip(); renderIndexCells(); renderPnlCell(); renderWatchlist(); renderPositions();
			// 盯盘提醒（一次预警：触发后标记 fired，保留价格，用户重置后重新武装）
			let firedAny = false;
			for (const p of state.positions) {
				const q = quoteOf(p.symbol);
				if (!Number.isFinite(q.price)) continue;
				if (p.fired !== "high" && Number.isFinite(p.alertHigh) && p.alertHigh > 0 && q.price >= p.alertHigh) {
					p.fired = "high"; firedAny = true;
					notifyAlert(q.name || p.symbol, p.symbol, "high", p.alertHigh, q.price);
				}
				if (p.fired !== "low" && Number.isFinite(p.alertLow) && p.alertLow > 0 && q.price <= p.alertLow) {
					p.fired = "low"; firedAny = true;
					notifyAlert(q.name || p.symbol, p.symbol, "low", p.alertLow, q.price);
				}
			}
			if (firedAny) { persistPositions(); renderPositions(); }
			renderAlertStatus(); renderPanelTabs(); }
		// ---- data refresh ----
		const remote = (() => { try { return ctx.get("remote"); } catch { return; } })();
	...
	const refreshQuotes = async () => {
		...
		const refreshWorkspaces = async () => {
			if (remote === void 0 || disposed) return;
			try { const list = await remote.session.list({}); if (!list.ok) return; if (disposed) return; wsCell.textContent = `工作区 ${list.value.items.length}`; } catch { wsCell.textContent = "工作区 --"; }
		};
		// ---- autocomplete ----
	...
	wlInput.addEventListener("input", () => {
		...
		const openPosForm = (symbol, qty, cost) => {
			closeDropdownPos();
			const pos = symbol ? state.positions.find((p) => p.symbol === symbol) : void 0;
			iSym.value = symbol ?? ""; iQty.value = qty !== void 0 ? String(qty) : ""; iCost.value = cost !== void 0 ? String(cost) : "";
			iAlertH.value = Number.isFinite(pos?.alertHigh) && pos.alertHigh > 0 ? String(pos.alertHigh) : "";
			iAlertL.value = Number.isFinite(pos?.alertLow) && pos.alertLow > 0 ? String(pos.alertLow) : "";
			posForm.hidden = false; posForm.dataset.mode = symbol ? "edit" : "add"; iSym.focus();
		};
		...
		const symbol = normalizeSymbol(iSym.value.trim()); const qty = Number(iQty.value); const cost = Number(iCost.value);
			...
			const alertH = iAlertH.value.trim() === "" ? NaN : Number(iAlertH.value);
			const alertL = iAlertL.value.trim() === "" ? NaN : Number(iAlertL.value);
			if (Number.isFinite(alertH) && alertH <= 0) { toast("盯盘高价需大于 0"); return; }
			if (Number.isFinite(alertL) && alertL <= 0) { toast("盯盘低价需大于 0"); return; }
			if (Number.isFinite(alertH) && Number.isFinite(alertL) && alertL >= alertH) { toast("盯盘低价应低于盯盘高价"); return; }
			const mode = posForm.dataset.mode;
			if (mode === "edit") { const existing = state.positions.find((p) => p.symbol === symbol); if (existing) { existing.qty = qty; existing.cost = cost; existing.alertHigh = alertH; existing.alertLow = alertL; existing.fired = void 0; } else { state.positions.push({ symbol, qty, cost, alertHigh: alertH, alertLow: alertL }); } }
			else { const existing = state.positions.find((p) => p.symbol === symbol); if (existing) { existing.qty = qty; existing.cost = cost; existing.alertHigh = alertH; existing.alertLow = alertL; existing.fired = void 0; toast(`${symbol} 已存在，已更新`); } else { state.positions.push({ symbol, qty, cost, alertHigh: alertH, alertLow: alertL }); } }
			ensureNotifPermission(); persistPositions(); closePosForm(); renderPositions(); renderPnlCell(); renderAlertStatus(); refreshQuotes();
		};
		...
		posTable.addEventListener("click", (event) => {
			...
			if (!tr) return; const symbol = tr.dataset.symbol;
			const btn = event.target.closest(".dsh-stk-btnDanger");
			if (btn) {
			...
			const alertEl = event.target.closest("[data-act='alert']");
			if (alertEl) openAlertPopup(symbol);
		});
	...
	function openStockPopup(symbol) {
		...
		const PERIODS = [["minute", "分时"], ["day", "日K"], ["week", "周K"], ["month", "月K"]];
		...
		const resizeHandle = document.createElement("div"); resizeHandle.className = "dsh-stk-modalResize"; resizeHandle.title = "拖拽调整大小";
		card.append(head, bodyBox, foot, resizeHandle);
		// 恢复持久化大小
		const MODAL_SIZE_KEY = "dsh.stock.modalSize.v1";
		try { const saved = localStorage.getItem(MODAL_SIZE_KEY); if (saved) { const ps = JSON.parse(saved); if (typeof ps.w === "number" && typeof ps.h === "number") { card.style.width = ps.w + "px"; card.style.height = ps.h + "px"; card.dataset.size = "1"; } } } catch {}
		// 拖拽调整大小
		...
		function startResize(e) {
			...
			function onMove(e2) {
				...
				const nw = Math.max(360, resizeStart.w + e2.clientX - resizeStart.x);
				const nh = Math.max(240, resizeStart.h + e2.clientY - resizeStart.y);
				card.style.width = Math.min(nw, window.innerWidth - 48) + "px";
				card.style.height = Math.min(nh, window.innerHeight - 96) + "px";
			...
			function onUp() {
				...
				document.removeEventListener("mousemove", onMove);
				document.removeEventListener("mouseup", onUp);
				try { localStorage.setItem(MODAL_SIZE_KEY, JSON.stringify({ w: card.offsetWidth, h: card.offsetHeight })); } catch {}
			}
			document.addEventListener("mousemove", onMove);
			document.addEventListener("mouseup", onUp);
		}
		...
		let minuteHover = -1;
		// 可视窗口（缩放/平移）：start=起始索引, count=可见根数, hover=悬浮bar绝对索引
		let popupView = { start: 0, count: 0, hover: -1 };
		let popupDrag = null;
		...
		const redraw = () => {
			if (popupEl !== backdrop || canvas.clientWidth <= 0) return;
			if (currentPeriod === "minute") { if (minuteCache !== null) drawMinuteChart(canvas, minuteCache, minuteHover); return; }
			if (barsCache !== null) { if (popupView.count === 0) popupView.count = barsCache.length; if (popupView.hover < 0) popupView.hover = barsCache.length - 1; drawKlineChart(canvas, barsCache, popupView, popupView.hover); }
		};
		popupResize = redraw;
		...
		// 滚轮缩放（锚点缩放：鼠标所在bar保持不动；分时为整日视图，不支持缩放）
		canvas.addEventListener("wheel", (event) => {
			if (currentPeriod === "minute" || barsCache === null || popupEl !== backdrop) return;
			event.preventDefault();
			const rect = canvas.getBoundingClientRect();
			const L = klineLayout(canvas, barsCache, popupView);
			const ratio = Math.max(0, Math.min(1, (event.clientX - rect.left - L.padL) / L.plotW));
			const oldCount = popupView.count;
			let newCount = event.deltaY < 0 ? oldCount * 0.82 : oldCount * 1.2;
			newCount = Math.round(Math.max(20, Math.min(barsCache.length, newCount)));
			if (newCount === oldCount) return;
			const anchor = popupView.start + Math.max(0, Math.min(oldCount, ratio * oldCount));
			popupView.start = Math.round(Math.max(0, Math.min(barsCache.length - newCount, anchor - ratio * newCount)));
			popupView.count = newCount;
			redraw();
		}, { passive: false });
		// 拖拽平移（K线）+ 十字光标（K线 hover / 分时 hover 共用）
		canvas.addEventListener("mousedown", (event) => { if (currentPeriod === "minute" || barsCache === null) return; popupDrag = { x: event.clientX, start0: popupView.start }; canvas.style.cursor = "grabbing"; });
		window.addEventListener("mousemove", (event) => {
			if (popupEl !== backdrop) return;
			if (currentPeriod === "minute") {
				if (minuteCache === null) return;
				const rect = canvas.getBoundingClientRect();
				const L = minuteLayout(canvas);
				const plotW = L.cssW - L.padL - L.padR;
				const i = Math.floor((event.clientX - rect.left - L.padL) / plotW * (minuteCache.points.length - 1));
				const h = (i >= 0 && i < minuteCache.points.length) ? i : -1;
				if (h !== minuteHover) { minuteHover = h; redraw(); }
				return;
			}
			if (barsCache === null) return;
			if (popupDrag !== null) {
				const rect = canvas.getBoundingClientRect();
				const L = klineLayout(canvas, barsCache, popupView);
				const dIdx = Math.round((event.clientX - popupDrag.x) / L.step);
				popupView.start = Math.max(0, Math.min(barsCache.length - popupView.count, popupDrag.start0 - dIdx));
				redraw();
			} else {
				const rect = canvas.getBoundingClientRect();
				const L = klineLayout(canvas, barsCache, popupView);
				const i = Math.floor((event.clientX - rect.left - L.padL) / L.step);
				const h = (i >= 0 && i < L.count) ? L.start + i : -1;
				if (h !== popupView.hover) { popupView.hover = h; redraw(); }
			}
		});
		window.addEventListener("mouseup", () => { if (popupDrag !== null) { popupDrag = null; canvas.style.cursor = ""; } });
		canvas.addEventListener("mouseleave", () => { const dirty = popupView.hover !== -1 || minuteHover !== -1; popupView.hover = -1; minuteHover = -1; if (dirty) redraw(); });
		...
		const load = async () => {
			if (disposed) return;
			if (currentPeriod === "minute") {
				msg.style.display = ""; showMsg("加载分时中…", false);
				try {
					const md = await fetchMinute(sym);
					if (disposed || seq !== popupSeq || popupEl !== backdrop) return;
					minuteCache = md; minuteHover = -1;
					msg.style.display = "none";
					redraw();
					const n = md.points.length;
					const last = md.points[n - 1];
					const rangePct = Number.isFinite(md.prevClose) && md.prevClose > 0 ? (last.p / md.prevClose - 1) * 100 : NaN;
					const mk = (text, color) => { const s = document.createElement("span"); if (color) s.style.color = color; s.textContent = text; return s; };
					foot.textContent = "";
					foot.append(
						mk(`分时 ${n}点`),
						mk(Number.isFinite(rangePct) ? `较昨收 ${rangePct > 0 ? "+" : ""}${rangePct.toFixed(2)}%` : "较昨收 --")
					);
				} catch (error) {
					if (disposed || seq !== popupSeq || popupEl !== backdrop) return;
					const reason = error instanceof Error ? error.message : "error";
					msg.style.display = "";
					showMsg(`${sym}：分时加载失败（${reason}）`, true);
				}
				return;
			}
		...
		if (bars.length === 0) throw new Error("kline-empty");
				if (disposed || seq !== popupSeq || popupEl !== backdrop) return;
				// 从实时行情拼今日 K 线柱（未开盘/休市时价格字段为 0，须跳过，
				// 否则全零柱会把价格轴拉扁——只在实际有有效行情时才拼）
				const q = quoteOf(sym);
				const todayStr = new Date().toISOString().slice(0, 10);
				const lastBar = bars.length > 0 ? bars[bars.length - 1] : null;
				if (Number.isFinite(q.open) && q.open > 0 && Number.isFinite(q.price) && q.price > 0) {
					const tb = { t: todayStr, o: q.open, h: Math.max(q.high, q.price, q.open, q.low), l: Math.min(q.low, q.price, q.open, q.high), c: q.price, v: q.volume || 0 };
					if (lastBar && lastBar.t === todayStr) {
						lastBar.h = Math.max(lastBar.h, tb.h);
						lastBar.l = Math.min(lastBar.l, tb.l);
						lastBar.c = tb.c;
						if (tb.v > 0) lastBar.v = tb.v;
					} else {
						bars.push(tb);
					}
				}
							barsCache = bars;
				msg.style.display = "none";
				popupView.start = 0; popupView.count = bars.length; popupView.hover = bars.length - 1;
				redraw();
				...
				foot.textContent = "";
				foot.append(
					mk((currentPeriod === "week" ? "周K" : currentPeriod === "month" ? "月K" : "日K") + " " + bars.length + " 根"),
	...
	// ---- 盯盘弹窗 ----
	let alertPopupEl = null, alertPopupKeydown = null;
	function closeAlertPopup() {
		if (alertPopupEl !== null) { alertPopupEl.remove(); alertPopupEl = null; }
		if (alertPopupKeydown !== null) { document.removeEventListener("keydown", alertPopupKeydown); alertPopupKeydown = null; }
	}
	function openAlertPopup(symbol) {
		if (disposed) return;
		const pos = state.positions.find((p) => p.symbol === symbol);
		if (pos === void 0) return;
		closeAlertPopup();
		const backdrop = document.createElement("div"); backdrop.className = "dsh-stk-modal dsh-stk-alertModal";
		const card = document.createElement("div"); card.className = "dsh-stk-alertCard";
		const head = document.createElement("div"); head.className = "dsh-stk-modalHead";
		const q0 = state.quotes.get(symbol);
		const nameEl = document.createElement("span"); nameEl.className = "dsh-stk-modalName";
		nameEl.textContent = q0 && q0.name && q0.name !== symbol ? q0.name : symbol;
		const symEl = document.createElement("span"); symEl.className = "dsh-stk-modalSym"; symEl.textContent = symbol;
		const closeBtn = document.createElement("button"); closeBtn.type = "button"; closeBtn.className = "dsh-stk-modalClose"; closeBtn.title = "关闭 (Esc)"; closeBtn.textContent = "✕";
		head.append(nameEl, symEl, closeBtn);
		const bodyBox = document.createElement("div"); bodyBox.className = "dsh-stk-alertBody";
		const rowHigh = document.createElement("div"); rowHigh.className = "dsh-stk-formrow";
		const lHigh = document.createElement("label"); lHigh.textContent = "盯盘高价"; lHigh.style.width = "58px";
		const iHigh = document.createElement("input"); iHigh.type = "number"; iHigh.min = "0"; iHigh.step = "any"; iHigh.placeholder = "现价≥此价提醒";
		iHigh.value = Number.isFinite(pos.alertHigh) && pos.alertHigh > 0 ? String(pos.alertHigh) : "";
		rowHigh.append(lHigh, iHigh);
		const rowLow = document.createElement("div"); rowLow.className = "dsh-stk-formrow";
		const lLow = document.createElement("label"); lLow.textContent = "盯盘低价"; lLow.style.width = "58px";
		const iLow = document.createElement("input"); iLow.type = "number"; iLow.min = "0"; iLow.step = "any"; iLow.placeholder = "现价≤此价提醒";
		iLow.value = Number.isFinite(pos.alertLow) && pos.alertLow > 0 ? String(pos.alertLow) : "";
		rowLow.append(lLow, iLow);
		const hint = document.createElement("div"); hint.className = "dsh-stk-alertHint";
		hint.textContent = pos.fired ? `已触发${pos.fired === "high" ? "高价" : "低价"}提醒，重新保存后重新武装。` : "留空表示不设该方向提醒；触发一次后需重置。";
		const foot = document.createElement("div"); foot.className = "dsh-stk-alertFoot";
		const saveBtn = document.createElement("button"); saveBtn.type = "button"; saveBtn.className = "dsh-stk-btn dsh-stk-btnPrimary"; saveBtn.textContent = "保存";
		const clearBtn = document.createElement("button"); clearBtn.type = "button"; clearBtn.className = "dsh-stk-btn dsh-stk-btnDanger"; clearBtn.textContent = "清除提醒";
		const cancelBtn = document.createElement("button"); cancelBtn.type = "button"; cancelBtn.className = "dsh-stk-btn"; cancelBtn.textContent = "取消";
		foot.append(saveBtn, clearBtn, cancelBtn);
		bodyBox.append(rowHigh, rowLow, hint);
		card.append(head, bodyBox, foot);
		backdrop.append(card);
		document.body.append(backdrop);
		alertPopupEl = backdrop;
		const close = () => { closeAlertPopup(); };
		alertPopupKeydown = (event) => { if (event.key === "Escape") { event.stopPropagation(); closeAlertPopup(); } };
		document.addEventListener("keydown", alertPopupKeydown);
		backdrop.addEventListener("click", (event) => { if (event.target === backdrop) closeAlertPopup(); });
		closeBtn.addEventListener("click", close);
		cancelBtn.addEventListener("click", close);
		const save = () => {
			const hv = iHigh.value.trim(), lv = iLow.value.trim();
			const alertHigh = hv === "" ? NaN : Number(hv);
			const alertLow = lv === "" ? NaN : Number(lv);
			if (hv !== "" && (!Number.isFinite(alertHigh) || alertHigh <= 0)) { toast("盯盘高价需大于 0"); return; }
			if (lv !== "" && (!Number.isFinite(alertLow) || alertLow <= 0)) { toast("盯盘低价需大于 0"); return; }
			if (Number.isFinite(alertHigh) && Number.isFinite(alertLow) && alertLow >= alertHigh) { toast("盯盘低价应低于盯盘高价"); return; }
			pos.alertHigh = alertHigh;
			pos.alertLow = alertLow;
			pos.fired = void 0;
			persistPositions(); renderPositions(); renderAlertStatus();
			ensureNotifPermission();
			toast(Number.isFinite(alertHigh) || Number.isFinite(alertLow) ? "已保存盯盘提醒" : "已清除盯盘提醒");
			closeAlertPopup();
		};
		saveBtn.addEventListener("click", save);
		clearBtn.addEventListener("click", () => { iHigh.value = ""; iLow.value = ""; save(); });
	}
	// ---- boot ----
