...
window.__ModuleLoader__.load({
	...
	function openStockPopup(symbol) {
		...
		const redraw = () => { if (popupEl === backdrop && barsCache !== null && canvas.clientWidth > 0) { if (popupView.count === 0) popupView.count = barsCache.length; if (popupView.hover < 0) popupView.hover = barsCache.length - 1; drawKlineChart(canvas, barsCache, popupView, popupView.hover); } };
		popupResize = redraw;
		...
		// 滚轮缩放（锚点缩放：鼠标所在bar保持不动）
		canvas.addEventListener("wheel", (event) => {
			if (barsCache === null || popupEl !== backdrop) return;
			event.preventDefault();
		...
		// 拖拽平移 + hover 十字光标
		canvas.addEventListener("mousedown", (event) => { if (barsCache === null) return; popupDrag = { x: event.clientX, start0: popupView.start }; canvas.style.cursor = "grabbing"; });
		window.addEventListener("mousemove", (event) => {
			if (popupEl !== backdrop || barsCache === null) return;
			if (popupDrag !== null) {
				const rect = canvas.getBoundingClientRect();
				const L = klineLayout(canvas, barsCache, popupView);
		...
		canvas.addEventListener("mouseleave", () => { if (popupView.hover !== -1) { popupView.hover = -1; redraw(); } });
