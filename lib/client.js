...
window.__ModuleLoader__.load({
	...
	function apply(ctx) {
		...
		const posChip = document.createElement("span"); posChip.className = "dsh-stk-posChip"; titlebar.append(posChip);
		// 设置齿轮按钮（标题栏右侧）
	...
	function renderTape() {
		...
		if (count === 0) { posChip.textContent = ""; return; }
		const trend = stats.totalPnl > 0 ? "up" : stats.totalPnl < 0 ? "down" : "";
		const pnl = `${stats.totalPnl >= 0 ? "+" : ""}${stats.totalPnl.toFixed(0)}`;
		posChip.textContent = `持仓 ${count}  ${pnl}`;
		if (trend) posChip.dataset.trend = trend; else delete posChip.dataset.trend;
	}
