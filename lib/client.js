...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			`.dsh-stk-chip{font:500 12px/1 var(--ds-font-family-code);font-variant-numeric:tabular-nums;white-space:nowrap;align-items:baseline;gap:5px;display:inline-flex}.dsh-stk-chipName{color:var(--stk-dim)}.dsh-stk-chipVal{color:var(--stk-text);font-weight:600}.dsh-stk-chipChg[data-trend=up]{color:var(--stk-up);font-weight:600}.dsh-stk-chipChg[data-trend=down]{color:var(--stk-down);font-weight:600}.dsh-stk-chipChg:not([data-trend]){color:var(--stk-flat)}.dsh-stk-posChip{font:500 12px/1 var(--ds-font-family-code);font-variant-numeric:tabular-nums;white-space:nowrap;align-items:center;gap:8px;margin-left:auto;padding-left:8px;border-left:1px solid var(--stk-border);display:flex}.dsh-stk-posChip .posItem{display:inline-flex;align-items:baseline;gap:3px}.dsh-stk-posChip .posItem[data-trend=up]{color:var(--stk-up)}.dsh-stk-posChip .posItem[data-trend=down]{color:var(--stk-down)}.dsh-stk-posChip .posItem:not([data-trend]){color:var(--stk-flat)}.dsh-stk-posChip .posItem .posSym{color:var(--stk-dim);font-size:11px}.dsh-stk-posChip[data-trend=up]{color:var(--stk-up)}.dsh-stk-posChip[data-trend=down]{color:var(--stk-down)}.dsh-stk-posChip:not([data-trend]){color:var(--stk-flat)}`,
	...
	async function fetchKline(symbol, count = 120, timeoutMs = 1e4) {
		const response = await fetch(`${STOCK_API_BASE}/kline?symbol=${encodeURIComponent(symbol)}&count=${count}`, { signal: timeoutSignal(timeoutMs) });
	...
	function renderTape() {
		...
		posChip.textContent = "";
		const items = state.positions.slice(0, 3);
		if (items.length === 0) return;
		for (const p of items) {
			const quote = state.quotes.get(p.symbol);
			const qty = Number(p.qty), cost = Number(p.cost);
			const item = document.createElement("span"); item.className = "posItem";
			const sym = document.createElement("span"); sym.className = "posSym"; sym.textContent = p.symbol;
			item.append(sym);
		...
		if (state.positions.length > 3) {
			const more = document.createElement("span"); more.className = "posItem"; more.style.color = "var(--stk-dim)";
			more.textContent = `等 ${state.positions.length} 只`;
			posChip.append(more);
		}
		if (Number.isFinite(stats.totalPnl) && Math.abs(stats.totalPnl) > 0.01) {
			const total = document.createElement("span"); total.className = "posItem";
			if (stats.totalPnl > 0) total.dataset.trend = "up"; else total.dataset.trend = "down";
			total.textContent = `总 ${stats.totalPnl >= 0 ? "+" : ""}${stats.totalPnl.toFixed(0)}`;
			posChip.append(total);
		}
	}
	...
	function openStockPopup(symbol) {
		...
		qline.append(priceEl, chgEl); head.append(nameEl, symEl, qline, closeBtn);
		...
		const load = async () => {
			...
			try {
				const bars = await fetchKline(sym, 120);
