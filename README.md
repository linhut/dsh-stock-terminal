...
| 📢 实时行情跑马灯 | 自选列表无缝循环滚动，hover 暂停；空自选时静默展示默认指数；**点击任一品种弹出个股日K图**（蜡烛 + MA5/MA10 + 成交量副图，红涨绿跌适配明暗主题，Esc/遮罩/✕ 关闭） |
| 📈 个股 K 线弹窗 | A股/港股/指数走腾讯前复权日K；美股在腾讯数据不足时自动回退 Yahoo；加密货币走 Binance 日K；外汇提示暂不支持。服务端 `/api/kline` 60s 缓存 + 在途去重 |
...
- **宿主端短 TTL 缓存 + 在途请求去重**：quotes 5s / suggest 30s 缓存，并发同参数请求共享同一次上游拉取（上限 128 条防无界增长），多面板轮询也不会打爆上游。
- **符号规范化**：`SH600519` 等大写市场前缀自动转小写（外汇对 `USD/CNY` 保持原样），不再被误判为加密货币或非法符号。
- **轮询调度**：客户端改为 setTimeout 链式调度（上一次完成再排下一次，慢响应不堆叠）+ 在途请求守卫；页面隐藏时暂停轮询，回到前台立即刷新。
- **跑马灯不闪跳**：行情刷新只原地更新数字，不重建 DOM，动画位置不被重置。
- **联想防竞态**：搜索建议加序号计数器，慢的旧请求不会覆盖新输入的结果；移除全局定时器泄漏。
